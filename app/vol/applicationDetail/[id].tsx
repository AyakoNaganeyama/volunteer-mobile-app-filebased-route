...
const Page = () => {
  const { wholeOpportunities } = useListingStore();
  ...
  useEffect(() => {
    const foundOpportunity = wholeOpportunities.find((opp) => opp.id === id);
    setOpportunity(foundOpportunity || null);
  }, [id, wholeOpportunities]);
  if (!opportunity) {
    ...
    <>
      ...
      >
        ...
        <View style={{ marginBottom: 100 }}>
          <TouchableOpacity
            onPress={() => {
              handleRedirect(opportunity.registrationFormUrl);
            }}
          >
            <Text
              style={{
